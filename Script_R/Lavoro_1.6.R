...
output_path <- "~/Desktop/Master/Text Mining/TextMining/immages_stem"
...
#####################################################################################################
################### Strategia 1
s1_ret <- lexicometric(msg_s1$message)
formattable(s1_ret)

################### Strategia 2
s2_ret <- lexicometric(msg_s2$message)
formattable(s2_ret)


#####################################################################################################

hist(as.Date(msg_s1$date), breaks = 10, xlab='Years')

nchar = nchar(as.character(msg_s1$message))
nchar = as.data.frame(nchar) 
msg_s1 = cbind(msg_s1,nchar)

summary(msg_s1$nchar)
sd(msg_s1$nchar)


library(ggplot2) 
ggplot(nchar, aes(x =nchar) ) +
  geom_histogram(aes( fill = ..count..) , binwidth = 25) + 
  scale_x_continuous(breaks = seq(0 , 5500, 1000) ,
                     limits=c(0 , 5500)) +
  scale_y_continuous(name = "Count") +
  geom_vline( xintercept = 145, size = 0.5 , colour = "#FF3721" , linetype = "dashed")
...
setwd(output_path)
################### Strategia 1
gen_wordcloud(msg_s1$message, 's1', '01', 20)

################### Strategia 2
gen_wordcloud(msg_s2$message, 's2', '02', 20)
...
###### Strategia 1
df_s1_10 <- subset.data.frame(msg_s1, no_rating %in% 10)
df_s1_20 <- subset.data.frame(msg_s1, no_rating %in% 20)
df_s1_30 <- subset.data.frame(msg_s1, no_rating %in% 30)
df_s1_40 <- subset.data.frame(msg_s1, no_rating %in% 40)
df_s1_50 <- subset.data.frame(msg_s1, no_rating %in% 50)

gen_wordcloud(df_s1_10$message, 's1_10', '01_1', 5)
gen_wordcloud(df_s1_20$message, 's1_20', '01_2', 5)
gen_wordcloud(df_s1_30$message, 's1_30', '01_3', 5)
gen_wordcloud(df_s1_40$message, 's1_40', '01_4', 10)
gen_wordcloud(df_s1_50$message, 's1_50', '01_5', 20)

###### Strategia 2; genera i dataframe raggruppando i messaggi in base al rating
df_s2_10 <- subset.data.frame(msg_s2, no_rating %in% 10)
df_s2_20 <- subset.data.frame(msg_s2, no_rating %in% 20)
df_s2_30 <- subset.data.frame(msg_s2, no_rating %in% 30)
df_s2_40 <- subset.data.frame(msg_s2, no_rating %in% 40)
df_s2_50 <- subset.data.frame(msg_s2, no_rating %in% 50)

gen_wordcloud(df_s2_10$message, 's2_10', '02_1', 5)
gen_wordcloud(df_s2_20$message, 's2_20', '02_2', 5)
gen_wordcloud(df_s2_30$message, 's2_30', '02_3', 5)
gen_wordcloud(df_s2_40$message, 's2_40', '02_4', 20)
gen_wordcloud(df_s2_50$message, 's2_50', '02_5', 20)
...
###### Genera i dataframe per i rating piu bassi e piu' alto
df_s2_low  <- subset.data.frame(msg_s2, no_rating %in% c(10, 20, 30) )
df_s2_high <- subset.data.frame(msg_s2, no_rating %in% c(50) )

#####################################################################################################

png('11_Hist_rating_s1.png', width = 800, height = 600)
#hist(msg_s1$no_rating, main='Histogram Restaurant messages Strategy 1', xlab = "Rating")
barplot(table(msg_s1$no_rating), col="steelblue", main='Histogram Restaurant messages Strategy 1', xlab = "Rating", ylab="Frequency")
dev.off()


png('12_Hist_rating_s2.png', width = 800, height = 600)
#hist(msg_s2$no_rating, main='Histogram Restaurant messages Strategy 2', xlab = "Rating")
barplot(table(msg_s2$no_rating), col="steelblue", main='Histogram Restaurant messages Strategy 2', xlab = "Rating", ylab="Frequency")
dev.off()
...
# Genera un corpus contenente i dataset per il rating basso e alto
df_corpus_s1 = Corpus(VectorSource(msg_s1$message))
...
dtm_s1_m <- removeSparseTerms(dtm_s1, 0.92) %>% as.matrix()
...
# Genera un corpus contenente i dataset per il rating basso e alto
df_corpus_s2 = Corpus(VectorSource(msg_s2$message))
...
# Costruisce una matrice rimuovendo i termini sparsi che appaiono con una frequenza minore dell'8%, 
# cioe' un valore di "sparsity" superiore al 92%.
dtm_s2_m <- removeSparseTerms(dtm_s2, 0.92) %>% as.matrix()
